...
async function getFeeds() {
  const res = await fetch("https://api.apify.com/v2/acts/shanes~tweet-flash/runs/last/dataset/items?token=" + process.env.APIFY_TOKEN, { next: { revalidate: 60 } });
  if (!res.ok) {
...
export default async function IndexPage() {
  const data = await getFeeds() as Tweet[];
  return (
